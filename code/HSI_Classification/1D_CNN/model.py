...
class Model():
    ...
    def __init__(self,args,sess):
        ...
        if args.use_lr_decay:
            ...
            self.lr = args.lr
        #print(self.cube_size,self.dim)
        self.image = tf.placeholder(dtype=tf.float32, shape=(None, self.cube_size,self.cube_size,274))#self.dim
        self.label = tf.placeholder(dtype=tf.float32, shape=(None, self.class_num))
        self.classifer = unit.classifer_share1d
        self.pre_label = self.classifer(self.image,self.class_num,self.cube_size,self.training,reuse=True)#tf.AUTO_REUSE
    ...
    def train(self,dataset):
        ...
        for i in range(self.iter_num):
            ...
            #print(self.loss_total,self.optimizer,self.merged,self.lr)
